...
async function upsertRestaurant(chatId, updates) {
  const { data, error } = await supabase
    .from('restaurants')
    .upsert({ chat_id: String(chatId), ...updates, updated_at: new Date() },
             { onConflict: 'chat_id' })
    .select()
    .single()
...
async function updateIngredientStock(restaurantId, ingredientName, newStock, reason = 'manual') {
  const { data: current } = await supabase
    ...
    .eq('restaurant_id', restaurantId).ilike('name', `%${ingredientName}%`).single()
  await supabase.from('ingredients')
    .update({ current_stock: newStock, last_updated: new Date() })
    .eq('restaurant_id', restaurantId).ilike('name', `%${ingredientName}%`)
  await supabase.from('stock_updates').insert({
    restaurant_id: restaurantId, ingredient_name: ingredientName,
    old_stock: current?.current_stock, new_stock: newStock, reason
  })
}

async function getWines(restaurantId) {
  const { data } = await supabase.from('wines').select('*').eq('restaurant_id', restaurantId)
  return data || []
}

async function bulkAddWines(restaurantId, wines) {
  const rows = wines.map(w => ({
    restaurant_id: restaurantId, name: w.name,
    normal_stock: w.normalStock || 12, current_stock: w.normalStock || 12,
...
async function updateWineStock(restaurantId, wineName, newStock) {
  await supabase.from('wines')
    .update({ current_stock: newStock, last_updated: new Date() })
    .eq('restaurant_id', restaurantId).ilike('name', `%${wineName}%`)
}

async function getSuppliers(restaurantId) {
  const { data } = await supabase.from('suppliers').select('*').eq('restaurant_id', restaurantId)
  return data || []
}

async function addSupplier(restaurantId, supplier) {
  const { data, error } = await supabase.from('suppliers')
    .insert({ restaurant_id: restaurantId, ...supplier }).select().single()
  if (error) throw error
  return data
}
...
async function addOrder(restaurantId, items) {
  const { data, error } = await supabase.from('orders')
    .insert({ restaurant_id: restaurantId, items, status: 'sent' }).select().single()
  if (error) throw error
...
async function getAllActiveRestaurants() {
  const { data } = await supabase.from('restaurants').select('*').eq('onboarding_complete', true)
  return data || []
}
...
module.exports = {
  supabase, getRestaurant, upsertRestaurant,
  getIngredients, bulkAddIngredients, updateIngredientStock,
  getWines, bulkAddWines, updateWineStock,
  getSuppliers, addSupplier,
  addOrder, getOrders,
  bulkAddMenuItems,
  getAllActiveRestaurants, getFullContext
}
